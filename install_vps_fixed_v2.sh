...
# ================================
# XBoard Modern VPS 智能安装脚本 (修复版 v2)
# 解决环境检测、前端构建和依赖安装问题
# ================================
...
install_nodejs() {
    ...
    if command -v node &> /dev/null; then
        ...
        # 检查版本是否满足要求
        if [ -n "$NODE_MAJOR_VERSION" ] && [ "$NODE_MAJOR_VERSION" -ge 14 ]; then
            log_success "Node.js版本满足要求: $NODE_VERSION"
            return 0
        elif [ -n "$NODE_MAJOR_VERSION" ] && [ "$NODE_MAJOR_VERSION" -ge 8 ]; then
            log_warning "Node.js版本较低: $NODE_VERSION，但可以继续使用"
            return 0
        elif [ -n "$NODE_MAJOR_VERSION" ]; then
            log_warning "Node.js版本过低: $NODE_VERSION，需要升级"
        else
            log_warning "无法检测Node.js版本，尝试升级"
        fi
    else
    ...
    if [ -n "$PYTHON_VERSION" ]; then
        ...
        if [ "$PYTHON_VERSION" = "3.6" ]; then
            if [ -f "backend/requirements_python36.txt" ]; then
                ...
                pip install "fastapi<0.84.0" "uvicorn<0.18.0" "sqlalchemy<1.5.0" "pydantic<2.0.0" python-multipart python-jose[cryptography] passlib[bcrypt] python-dotenv email-validator
            fi
            ...
            if [ -f "backend/requirements_python37.txt" ]; then
                ...
                pip install "fastapi<0.95.0" "uvicorn<0.21.0" "sqlalchemy<2.0.0" "pydantic<2.0.0" python-multipart python-jose[cryptography] passlib[bcrypt] python-dotenv email-validator
            fi
...
main() {
    echo "=========================================="
    echo "🚀 XBoard Modern VPS 智能安装脚本 (修复版 v2)"
    echo "=========================================="
    echo ""
