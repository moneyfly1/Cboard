...
# 读取原始文件
with open('/Users/apple/Downloads/xboard 2/app/api/api_v1/endpoints/users.py', 'r', encoding='utf-8') as f:
    content = f.read()

# 新的仪表盘API函数
new_dashboard_function = '''@router.get("/dashboard-info", response_model=ResponseBase)
...
# 使用正则表达式找到并替换仪表盘API函数
pattern = r'@router\.get\("/dashboard-info".*?def get_user_dashboard_info.*?\)\s*:\s*Any:.*?(?=@router\.|$)'
replacement = new_dashboard_function

# 替换函数
new_content = re.sub(pattern, replacement, content, flags=re.DOTALL)

# 写入新文件
with open('/Users/apple/Downloads/xboard 2/app/api/api_v1/endpoints/users_fixed.py', 'w', encoding='utf-8') as f:
    f.write(new_content)
